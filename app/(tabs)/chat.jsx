...
const Message = ({ text, isSent, time }) => (
  <View className={`flex-row ${isSent ? "justify-end" : "justify-start"} mb-3`}>
    <View
      className={`${
        isSent ? "bg-[#9f86ff]" : "bg-white"
      } px-4 py-2 rounded-2xl max-w-[80%]`}
    >
      <Text
        className={`${isSent ? "text-white" : "text-[#4a3b6b]"} font-pmedium`}
      >
        {text}
      </Text>
      <Text
        className={`${
          isSent ? "text-[#f0e6ff]" : "text-[#6f5c91]"
        } text-xs mt-1`}
      >
        {time}
      </Text>
...
const Chat = () => {
  ...
  const [messages, setMessages] = useState([
    ...
    {
      text: "I'm great! Are you interested in joining the trip to Qutub Minar?",
      isSent: false,
      time: "10:32 AM",
    },
    {
      text: "Yes, I'd love to! What time are you planning to go?",
      isSent: true,
      time: "10:33 AM",
    },
  ]);
  ...
  const sendMessage = () => {
    if (message.trim()) {
      ...
      setTimeout(() => {
        const reply = {
          text: "That sounds great! Let's do badmosi",
          isSent: false,
          time: new Date().toLocaleTimeString([], {
            hour: "2-digit",
            minute: "2-digit",
  ...
  return (
    ...
    >
      ...
      <View
        className={`px-6 ${keyboardVisible ? "pb-2" : "pb-6"} pt-2 bg-white`}
      >
