...
export const getFriendRequests = async () => {
  ...
  return requestersData.map((requester) => ({
    ...
    mutualFriends: 0, // You can implement this later
  }));
};
...
export const acceptFriendRequest = async (requesterId) => {
  ...
  const { data: user, error: userError } = await supabase
    .from("users")
    .update({
      friends: supabase.sql`array_append(friends, ${requesterId})`,
  ...
  const { error: requesterError } = await supabase
    .from("users")
    .update({
      friends: supabase.sql`array_append(friends, ${session.user.id})`,
    })
...
export const sendFriendRequest = async (recipientId) => {
  console.log("sendFriendRequest", recipientId);
  const {
    data: { session },
    error: sessionError,
  } = await supabase.auth.getSession();
  ...
  console.log([...currentRequests, session.user.id]);
